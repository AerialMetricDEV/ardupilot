...
void AP_Parachute::set_sink_rate(float sink_rate,float relative_alt_parachute_m,bool in_vtol)
{
    ...
    // if drone is sinking too fast, update loop_time_ms
    if(in_vtol){
...
void AP_Parachute::check_sink_rate()
{
    ...
    if ((_sink_time_ms > 0) && ((AP_HAL::millis() - _sink_time_ms) > loop_time_ms)) {
        release();
    }
}
